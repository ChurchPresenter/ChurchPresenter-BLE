...
2b. If a sticky-log-*.jsonl is present, ALSO run stickyAudit (no DB needed, seconds to run) —
    ...
    (never drifts out of sync as the table grows), grouped by how well-supported the new book was:
        ...
        CHAPTER-CLEARED       — same book, chapter nulled — the exact shape of an earlier pass
...
compiled, passed, and had silently stopped exercising their gate (an earlier pass).
...
Service-level replay exists since an earlier pass: `DbReplayTest` replays an archived `.db`
...
**Mechanism-level tests** (an earlier pass): a fix that generalizes a
...
| Per-word ASR confidence unused | `words_json`/`live_word_confidences` reach neither app nor engine | STT payload → `SttUpdate` | **OPEN, measured** — segment-aggregate confidence does NOT separate right from wrong detections (an earlier pass). Only a token-local signal could work; see SESSIONS.md before re-testing |
...
| `triage_report.py` scored a late confirmation as FN *and* FP | engine names the verse +11s after the operator clicks | `tools/triage_report.py` | **FIXED** — new LATE bucket + Coverage metric; an earlier pass read 47% recall at 85% coverage |
| MT renders «Плач Иеремии» as two book names joined by "and" | db row 160: "book of Lamentations and Jeremiah, chapter 3" | machine translation, upstream | **OPEN** (FP) — the one surviving wrong-book emission in an earlier pass; self-corrects in ~1s |
...
| Short real-word aliases on the EN track | "am"→Amos and similar 2-3 char forms in prose | `BookResolver` per-language scoping | **OPEN** (FP) — raising `SHORT_ALIAS_MAX_LEN` to 5 was swept in an earlier pass and rejected: it kills a real explicit `От Иоанна 3:6` |
...
| Sequential verse-by-verse reading latency | one verse per operator click, 4–15 s apart; engine confirms 5–10 s late | `ContinuationEngine` / `Stabilizer` | **PARTIAL** (verse-side coverage) + user-facing "Verse speed" knob an earlier pass. Residual is STT segment-finalization latency, outside the engine |
...
| `chapter-history` costs precision and earns nothing | 1 TP in 8 services, never accepted, 94 emissions/0 TP in an earlier pass-08 | `Config.chapterHistoryEnabled` | **RETIRED** — flag off by default; removing it dropped 2 emissions across 8 services with zero recall loss |
| `replayEval` compared the matched module's own numbering | ground truth is canonical since an earlier pass; two RST modules here number Psalms differently | `ReplayEval.sameVerse` | **FIXED** — canonical comparison, legacy display rows still matched the old way |
...
- **chapter-history** — **OFF since an earlier pass** (`Config.chapterHistoryEnabled = false`; see the gap
...
- **Russian-validated, not English-validated.** Every gate above was tuned against Russian sermon
  ...
  the bugs found in the one such session (an earlier pass) trace specifically to English-source
...
- **Tiered auto-follow changes the stakes per matchType.** `explicit`, `continuation` and — since
  an earlier pass — `chapter-scan` push to the output screen automatically; only `reverse` stages the
...
Chapter and verse were display-numbered until an earlier pass, which scored every Psalm as an FN (Synodal
...
**When triaging a recording made before an earlier pass**: its logs are display-numbered — Psalms off by
